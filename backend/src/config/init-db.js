...
const SERVICES_V2 = [
  { id: 1, name: 'Giúp việc theo giờ', price: 60000, slug: 'giup-viec-theo-gio',
  ...
  { id: 2, name: 'Giúp việc định kỳ', price: 55000, slug: 'giup-viec-dinh-ky',
  ...
  { id: 3, name: 'Nấu ăn gia đình', price: 70000, slug: 'nau-an-gia-dinh',
  ...
  { id: 4, name: 'Trông trẻ tại nhà', price: 70000, slug: 'trong-tre-tai-nha',
  ...
  { id: 5, name: 'Chăm sóc người cao tuổi', price: 75000, slug: 'cham-soc-nguoi-cao-tuoi',
  ...
  { id: 6, name: 'Tổng vệ sinh (Deep Clean)', price: 15000, slug: 'tong-ve-sinh-deep-clean',
...
const NEW_SERVICES_V2 = [
  { name: 'Vệ sinh Sofa, Nệm & Rèm', price: 250000, slug: 've-sinh-sofa-nem-rem',
  ...
  { name: 'Vệ sinh Điều hòa', price: 150000, slug: 've-sinh-dieu-hoa',
  ...
  { name: 'Vệ sinh Máy giặt & Thiết bị bếp', price: 250000, slug: 've-sinh-may-giat-thiet-bi-bep',
...
async function migrateServicesV2(connection) {
  ...
  for (const s of SERVICES_V2) {
    await connection.query(
      'UPDATE services SET service_name=?, description=?, short_description=?, base_price=?, slug=? WHERE service_id=?',
      [s.name, s.desc, s.short, s.price, s.slug, s.id]
    );
  }
  // INSERT 6 dịch vụ mới (idempotent qua slug UNIQUE)
  for (const s of NEW_SERVICES_V2) {
    await connection.query(
      'INSERT IGNORE INTO services (service_name, description, short_description, base_price, slug, is_active) VALUES (?,?,?,?,?,TRUE)',
      [s.name, s.desc, s.short, s.price, s.slug]
    );
  }
  // Cập nhật short_description cho dịch vụ mới đã tồn tại (khi INSERT IGNORE bỏ qua)
  for (const s of NEW_SERVICES_V2) {
    await connection.query(
