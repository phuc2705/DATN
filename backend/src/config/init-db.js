...
async function migrateServicesV2(connection) {
  ...
  for (const s of SERVICES_V2) {
    await connection.query(
      'UPDATE services SET service_name=?, description=?, base_price=?, slug=? WHERE service_id=?',
      [s.name, s.desc, s.price, s.slug, s.id]
    );
  }
  // INSERT 6 dịch vụ mới (idempotent qua slug UNIQUE)
  for (const s of NEW_SERVICES_V2) {
    await connection.query(
      'INSERT IGNORE INTO services (service_name, description, base_price, slug, is_active) VALUES (?,?,?,?,TRUE)',
      [s.name, s.desc, s.price, s.slug]
    );
  }
}
