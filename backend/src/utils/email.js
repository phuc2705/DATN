// Tiện ích gửi email sử dụng Resend API (HTTPS, không bị Render block)
const { Resend } = require('resend');

const resend = new Resend(process.env.RESEND_API_KEY);

const OTP_HTML = (fullName, otpCode) => `
...
const sendOtpEmail = async (toEmail, otpCode, fullName) => {
  const from = process.env.RESEND_FROM || 'CleanConnect <onboarding@resend.dev>';
  await resend.emails.send({
    from,
    to: toEmail,
    subject: '[CleanConnect] Mã xác nhận đăng ký tài khoản',
    html: OTP_HTML(fullName, otpCode),
  });
};
